...
class Side {
	...
	async encode(file) {
		const data = this.map;
		this.encodeMetadata(data);
		await this.map.encode(data, file);
	}

	/**
		* Loads and returns the metadata for this side
		* @param {Object} data - dictionary encoded {@link Map} data for the side
		* @returns {Object)
		*/
	loadMetadata(data) {
		const res = {};
	...
	/**
		* Encodes the metadata for this side
		* @param {Object} target - The object to which the encoded metadata wil be assigned
		* @returns {null}
		*/
	encodeMetadata(target) {
