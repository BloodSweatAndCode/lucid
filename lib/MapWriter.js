const debug = require('debug')('MapWriter');
const fs = require('fs');
...
class MapWriter {
	...
	writeUInt8(val) { this.stream.write(val); }
	writeUInt16(val) { this.stream.write(Buffer.from(Uint16Array.from([val]))); }
	...
	async writeMap(dstFile) {
		...
		const lookupDict = seenLookup.reduce((a,c,i) => a[c] = i, {});
