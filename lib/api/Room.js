...
class Room {
	...
	static deserialize(data = {}) {
		return new Room({}, data, {
			entities: (data.entities || []).map(({ name, data, id }) => new Entity(name, data, id)),
			triggers: (data.triggers || []).map(({ name, data, id }) => new Trigger(name, data, id)),
			...
			bgTiles: ((data.bgTiles && data.bgTiles.data) || []).map(t => new Tiles(t)),
			fgTiles: ((data.fgTiles && data.fgTiles.data) || []).map(t => new Tiles(t)),
			objTiles: ((data.objTiles && data.objTiles.data) || []).map(t => new ObjectTiles(t))
		});
