...
class Map {
	...
	static deserialize(data = {}) {
		return new Map({
			...
			rooms: (data.rooms || []).map(r => Room.deserialize(r)),
			style: Style.deserialize(data.style),
			filers: (data.fillers || []).map(({ x, y, w, h }) => new Filler(x, y, w, h))
		});
	}
	...
	getBounds(xOffset = 0, yOffset = 0) {
		...
		for (let room of this.rooms) {
			...
			if (roomBounds.x1 < bounds.x1) { bounds.x1 = roomBounds.x1; }
			if (roomBounds.y1 < bounds.y1) { bounds.y1 = roomBounds.y1; }
			if (roomBounds.x2 > bounds.x2) { bounds.x2 = roomBounds.x2; }
			if (roomBounds.y2 > bounds.y2) { bounds.y2 = roomBounds.y2; }
		}
		return {
			x1: bounds.x1 + xOffset,
			y1: bounds.y1 + yOffset,
			x2: bounds.x2 + xOffset,
			y2: bounds.y2 + yOffset
		};
	}
