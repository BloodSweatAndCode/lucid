...
class Entity {
	...
	clone() {
		const skipList = blacklistedEntityAttrs.concat([ 'name', 'id', 'data' ]);
		const { data, name } = this;
		const newData = {};
		const keys = Object.keys(data).filter(k => !skipList.includes(k));

		// populate new entity's darta
		for (let key of keys) {
		...
		if (data.nodes && data.nodes.length) {
			newData.nodes = [];
			for (let node of data.nodes) {
				newData.nodes.push([node[0], node[1]]);
			}
		}

		// instantiate new entity based on this one
		return new (require('./entities/' + classMap[name].class))(newData);
	}
}
