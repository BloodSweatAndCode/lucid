...
class Entity {
	...
	draw2(layer, imgData, room, storage, path = this.spritePath()) {
		if (path == null) {
			throw new Error(`No sprite path for entity ${JSON.stringify(this)}`);
		}
		const { rx = 0.5, ry = 0.5 } = this.constructor.sprite || {};
		this.drawSprite2(layer, imgData, room, storage, { path, rx, ry });
	}

	drawSprite2(layer, imgData, room, storage, spriteObj) {
		const { path, rx = 0.5, ry = 0.5 } = spriteObj;
		const { width, height, offestX, offsetY, realWidth, realHeight } = imgData.meta[path].sprite;
		layer.getContext()._context.imageSmoothingEnabled = false;
		const image = new Image();
		image.onload = () => {
			const img = new Konva.Image({
				image,
				x: Math.floor(room.position[0] + this.data.x - (realWidth * rx + offestX)),
				y: Math.floor(room.position[1] + this.data.y - (realHeight * ry + offsetY)),
				width,
				height
			});
			layer.add(img);
		...
		image.src = join(storage.images, path.replace(/\//g, '_') + '.png');
	}
