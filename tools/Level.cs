...
// MVID: 3D796987-8450-4B63-AD59-3421CFAF60F2
// Assembly location: C:\Program Files (x86)\Steam\steamapps\common\Celeste\Celeste-otig.exe
...
namespace Celeste
{
  public class Level : Scene, IOverlayHandler
  {
        ...
        if (!this.Session.DoNotLoad.Contains(entityId1) && !entityIdList1.Contains(entityId1))
        {
          switch (entity2.Name)
          {
            ...
            case "birdForsakenCityGem":
              ...
              continue;
            case "birdPath":
              this.Add((Entity) new BirdPath(entityId1, entity2, vector2_1));
              continue;
            case "blackGem":
            ...
            case "cassette":
              ...
              continue;
            case "cassetteBlock":
              CassetteBlock cassetteBlock = new CassetteBlock(entity2, vector2_1, entityId1);
            ...
            case "conditionBlock":
              ...
              switch (conditionBlockModes)
              {
                case Level.ConditionBlockModes.Key:
                  flag4 = this.Session.DoNotLoad.Contains(none1);
                  break;
                case Level.ConditionBlockModes.Button:
                  flag4 = this.Session.GetFlag(DashSwitch.GetFlagName(none1));
                  break;
                case Level.ConditionBlockModes.Strawberry:
                  flag4 = this.Session.Strawberries.Contains(none1);
                  break;
                default:
            ...
            case "crumbleWallOnRumble":
              this.Add((Entity) new CrumbleWallOnRumble(entity2, vector2_1, entityId1));
              continue;
            case "crushBlock":
            ...
            case "lightbeam":
              ...
              continue;
            case "lightning":
              if (entity2.Bool("perLevel", false) || !this.Session.GetFlag("disable_lightning"))
              {
            ...
            case "npc":
              ...
              if (lower == "granny_07x")
              {
                this.Add((Entity) new NPC07X_Granny_Ending(entity2, vector2_1, false));
                continue;
              }
            ...
            case "ridgeGate":
              ...
              if (entity2.Attr("strawberries", "").Length > 0)
              {
              ...
              List<EntityID> entityIdList3 = new List<EntityID>();
              if (entity2.Attr("keys", "").Length > 0)
              {
                string str1 = entity2.Attr("keys", "");
                char[] chArray1 = new char[1]{ ',' };
                foreach (string str2 in str1.Split(chArray1))
                {
                  EntityID none2 = EntityID.None;
                  ...
                  none2.Level = strArray2[0];
                  none2.ID = Convert.ToInt32(strArray2[1]);
                  entityIdList3.Add(none2);
                }
              }
              foreach (EntityID entityId2 in entityIdList3)
              {
                if (!this.Session.Keys.Contains(entityId2))
                {
                  flag8 = false;
                  break;
                }
        ...
        if (!this.Session.DoNotLoad.Contains(id))
        {
          switch (trigger.Name)
          {
            ...
            case "cameraTargetTrigger":
              string flag4 = trigger.Attr("deleteFlag", "");
              if (string.IsNullOrEmpty(flag4) || !this.Session.GetFlag(flag4))
              {
        ...
        if (num1 != 10)
        {
          if ((HandleBase) Level.AssistSpeedSnapshot == (HandleBase) null || Level.AssistSpeedSnapshotValue != num1)
          {
            ...
            if (Level.AssistSpeedSnapshotValue < 10)
              Level.AssistSpeedSnapshot = Audio.CreateSnapshot("snapshot:/assist_game_speed/assist_speed_" + (object) (Level.AssistSpeedSnapshotValue * 10), true);
            else if (Level.AssistSpeedSnapshotValue <= 16)
              Level.AssistSpeedSnapshot = Audio.CreateSnapshot("snapshot:/variant_speed/variant_speed_" + (object) (Level.AssistSpeedSnapshotValue * 10), true);
          }
        }
    ...
    public override void Render()
    {
      ...
      if (!this.Paused || !this.PauseMainMenuOpen || (!Celeste.Input.MenuJournal.Check || !this.AllowHudHide))
    ...
    private void GiveUp(int returnIndex, bool restartArea, bool minimal, bool showHint)
    {
      ...
      if (restartArea & showHint)
        this.Add((Entity) (quickHint = new QuickResetHint()));
      ...
      menu.Add((TextMenu.Item) new TextMenu.Header(Dialog.Clean(restartArea ? "menu_restart_title" : "menu_return_title", (Language) null)));
      menu.Add(new TextMenu.Button(Dialog.Clean(restartArea ? "menu_restart_continue" : "menu_return_continue", (Language) null)).Pressed((Action) (() =>
      {
        Engine.TimeRate = 1f;
        menu.Focused = false;
      ...
      menu.Add(new TextMenu.Button(Dialog.Clean(restartArea ? "menu_restart_cancel" : "menu_return_cancel", (Language) null)).Pressed((Action) (() => menu.OnCancel())));
    ...
    private void AssistMode(int returnIndex, bool minimal)
    {
      ...
      TextMenu.Option<int> option2 = option1 = new TextMenu.Slider(label, (Func<int, string>) (i =>
      {
        ...
        return i == 1 ? Dialog.Clean("MENU_ASSIST_AIR_DASHES_TWO", (Language) null) : Dialog.Clean("MENU_ASSIST_AIR_DASHES_INFINITE", (Language) null);
      ...
      menu.Add((TextMenu.Item) new TextMenu.OnOff(Dialog.Clean("MENU_ASSIST_DASH_ASSIST", (Language) null), SaveData.Instance.Assists.DashAssist).Change((Action<bool>) (on => SaveData.Instance.Assists.DashAssist = on)));
    ...
    private void VariantMode(int returnIndex, bool minimal)
    {
      ...
      menu.Add((TextMenu.Item) new TextMenu.OnOff(Dialog.Clean("MENU_VARIANT_PLAYASBADELINE", (Language) null), SaveData.Instance.Assists.PlayAsBadeline).Change((Action<bool>) (on =>
      {
      ...
      bool skipCompleteScreen = false)
    {
      ...
      this.PauseLock = true;
      Action onComplete = !(AreaData.Get(this.Session).Interlude | skipCompleteScreen) ? (Action) (() => Engine.Scene = (Scene) new LevelExit(LevelExit.Mode.Completed, this.Session, (HiresSnow) null)) : (Action) (() => Engine.Scene = (Scene) new LevelExit(LevelExit.Mode.CompletedInterlude, this.Session, this.HiresSnow));
